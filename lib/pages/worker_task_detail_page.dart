...
class WorkerTaskDetailPage extends StatefulWidget {
  ...
  final bool isEditable; // Nuevo: indica si la tarea es editable o solo de vista.
...
class _WorkerTaskDetailPageState extends State<WorkerTaskDetailPage> {
  ...
  String? _currentEstado; // Estado actual de la tarea, editable por el trabajador.
  final TextEditingController _observacionesController = TextEditingController();
  List<String> _photoUrls = []; // Lista de URLs de fotos de evidencia.

  // Lista de estados posibles para una tarea.
  static const List<String> _estados = ['pendiente', 'en progreso', 'completada', 'cancelada'];
  ...
  String? _observacionesIniciales; // Observaciones del trabajador guardadas previamente.
  ...
  bool _initialMapMoved = false; // Bandera para asegurar que el mapa solo se mueva una vez.
  ...
  void _loadTaskDetails() {
    final data = widget.taskDocument.data() as Map<String, dynamic>?; // Acceso seguro a los datos.
    ...
    _adminId = data['admin_asignador_id']; // Asumiendo que hay un campo para el ID del admin.
  ...
  Future<void> _fetchAdminName() async {
    if (_adminId != null) {
      try {
        final adminDoc = await _firestore.collection('usuarios').doc(_adminId).get();
        if (adminDoc.exists) {
          final data = adminDoc.data();
          setState(() {
            _adminName = '${data?['nombres'] ?? ''} ${data?['apellidos'] ?? ''}';
          });
        } else {
          setState(() {
            _adminName = 'Administrador desconocido';
          });
          print('Advertencia: Documento del administrador con ID $_adminId no encontrado.');
        }
      } catch (e) {
        setState(() {
  ...
  Future<void> _updateTaskStatus() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        Navigator.pop(context); // Regresar a la página anterior después de actualizar.
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _launchMapUrl(double lat, double lng) async {
    ...
    if (await url_launcher.canLaunchUrl(url)) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No se pudo abrir el mapa. Asegúrate de tener Google Maps instalado.')),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.isEditable ? 'Editar Tarea' : 'Detalles de la Tarea'),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildInfoRow(Icons.info_outline, 'Estado Actual:', _currentEstado!.capitalize()),
              const SizedBox(height: 10),
              ...
              if (widget.isEditable) ...[
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: _estados.map((estado) {
                    return DropdownMenuItem(
                      value: estado,
                      child: Text(estado.capitalize()), // Capitalizar para mostrar
                    );
                  }).toList(),
                ...
                TextField(
                  ...
                  maxLines: 4,
                  enabled: widget.isEditable, // Deshabilita el TextField si no es editable
                ...
                Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: widget.isEditable ? _takePhoto : null, // Deshabilita el botón si no es editable
                      tooltip: widget.isEditable ? 'Tomar foto de evidencia' : 'No se pueden añadir fotos',
                    ),
                  ],
                ),
                const SizedBox(height: 8),
                      ...
                      itemBuilder: (context, index) {
                        return Stack(
                          children: [
                            ...
                            if (widget.isEditable) // Solo permite eliminar si es editable
                              Positioned(
                                top: 0,
                                right: 0,
                                child: GestureDetector(
                                  onTap: () {
                                    setState(() {
                                      _photoUrls.removeAt(index); // Permite eliminar fotos de la lista.
                                    });
                                  },
                                  child: Container(
                        ...
                        SizedBox(
                          ...
                          child: FlutterMap(
                            ...
                            options: MapOptions(
                              center: _ubicacion!,
                              zoom: 15,
                              interactiveFlags: InteractiveFlag.none, // Deshabilita la interacción para ver el mapa estático.
                              onMapReady: () {
              ...
              if (widget.isEditable) // Solo muestra el botón de guardar si es editable
                ElevatedButton(
